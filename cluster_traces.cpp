...
double peaks_diff=2.0; 			// Distances are computed only between traces that differ in peaks by no more than this number;
...
int print_stdout=0;			// Print the reason for a trace to be discarded in format: "so" tr_no <reason explained>
...
void cut_trace(vector<double> & data_x,vector<double> & data_f, int& start_f, int& tr_len_f, int& tr_no){
  ...
  for (int i=0;i<Ltr;i++) {
    if (data_f[i]>abnormal_f){
      if(print_stdout) {cout << "so " << tr_no << " this traces contains abnormal force" << endl;}
      tr_len_f=0;
      break;
    }
    if (data_x[i]>abnormal_x){
      if(print_stdout) {cout << "so " <<tr_no << " this trace contains abnormal extension" << endl;}
      tr_len_f=0;
      break;
    }
  }
  ...
  if (c==0 || c<n_cont ) {
    if(print_stdout) {cout << "so " <<tr_no << " this is a completely negative trace" << endl;}
    start_f=0;
    tr_len_f=0;
    return;
  }
  ...
  if (sigma_tail0>sigma_tail) {
      tr_len_f=0;
      if(print_stdout) {cout << "so " <<tr_no << " this trace has strange tail" << endl;}
  }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void compute_w(vector<double> &  xx, vector<double> &  ff, vector<double> &  ww, int& tr_no, int& tr_len, double& Npeaks){
  ...
  for(int i=0;i<xx.size();i++){
    if(ff[i]>F_min && ff[i]<F_max){
      ...
      complex<double> gamma=pow((pow(omega,3)-216.+12.*sqrt(324.-3*pow(omega,3))),(1./3.));
  ...
  for (int i=1;i<psize;i++){
     if(prob[i]<prob[0] && found_start==0){
        Lcstart=bincenters[i];
        found_start=1;
     }
  }
  if(found_start==0)Lcstart=bincenters[psize-1];
  ...
  if(df<=0){		
    ...
    if(print_stdout) {cout << "so " <<tr_no << " " << tr_len<< " the last point of prob is a maximum"<<endl;}
    tr_len=0;
  }
  ...
  if(n_mins-n_maxs != 1 && n_mins==2){
    if(print_stdout) {cout << "so " <<tr_no << " " << tr_len << " wrong min max assignation"<<endl;}
    tr_len=0;
  }
	   ...
	   for (int i=0;i<tr_len;i++){
        ...
        if(print_stdout) {
           if (tr_len*dx_interp<=len_min){
              cout << "so " <<tr_no << " " << tr_len << " this trace is too short" << endl;
           }   
        } 
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////        
	// Compute w score.
	  ...
	  ratio=ww_sum/tr_len;
        ...
        if(print_stdout) {
           if (tr_len*dx_interp>len_min && ratio<score_thresh){
              cout << "so " <<tr_no << " " << ratio << " this trace has ratio<0.5" << endl;
           }
        }
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#pragma omp for
	...
	DP[i].dist_nn[k]=truedist[k].dist;
      ...
      DP[i].rho=DP[i].rho*(global_score[i]/Length_tr_nm[i]);  
    }
 }
  //////////////////////////////////////////////////////////////////////////
  // Compute delta.
  ...
  cout << "#COL1 \t Trace number" << endl; 
  cout << "#COL2 \t Length [nm]" << endl; 
  cout << "#COL3 \t Quality score" << endl; 
  cout << "#COL4 \t Number of peaks" << endl; 
  cout << "#COL5 \t Cluster number (undefined if COL6>r_cut)" << endl;
  cout << "#COL6 \t Distance from cluster center" << endl;
  for(int i=0;i<num_traces;i++){
	  ...
	  if (DP[i].cl==j && DP[i].dist_center<=r_cut && DP[i].dist_center<DP[i].dist_other){
	     cout << "tr "<< orig_index[i]<<"\t"<< Length_tr[i] << "\t" << global_score[i] << "\t" << n_peaks[i] << "\t" << DP[i].cl << "\t" << DP[i].dist_center << endl;
          }
          else if (DP[i].cl==j){
               DP[i].cl=0;
               cout << "tr "<< orig_index[i]<<"\t"<< Length_tr[i] << "\t" << global_score[i] << "\t" << n_peaks[i] << "\t" << DP[i].cl << "\t" << DP[i].dist_center <<  endl;
      ...
      //if(DP[i].dist_center>r_cut && DP[i].dist_center>=DP[i].dist_other) DP[i].cl=0; 
      //cout << "tr "<< orig_index[i]<<"\t"<< Length_tr[i] << "\t" << global_score[i] << "\t" << n_peaks[i] << "\t" << DP[i].cl << "\t" << DP[i].dist_center <<  "\t" << DP[i].dist_other<< endl;
  }
  ...
  for(int i=0;i<num_traces;i++){
     ...
     if (DP[i].cen==1){
        int cl_size=0;
        int cl0=DP[i].cl;
        cout << "cluster " << cl0 << ": center= " << orig_index[i]  << " members: ";
        for(int k=0;k<num_traces;k++){
       	    if (DP[k].cl==cl0) { 
               cout << orig_index[k] << " "; 
               cl_size++;
